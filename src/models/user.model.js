...
// NEW: Method untuk get user reviews dengan pagination
userSchema.methods.getReviews = function (options = {}) {
  const Review = require("./review.model");
  return Review.getByUser(this._id, options);
};

// NEW: Method untuk check apakah user sudah review produk tertentu
userSchema.methods.hasReviewedProduct = async function (productId) {
  const Review = require("./review.model");
  const result = await Review.checkUserCanReview(this._id, productId);
  return result.hasExistingReview;
};

userSchema.methods.createReview = async function (productId, rating, comment) {
  const Review = require("./review.model");
  return await Review.createOrUpdateReview(this._id, productId, rating, comment);
};

// NEW: Method untuk get user review statistics
userSchema.methods.getReviewStats = async function () {
  const Review = require("./review.model");

  const stats = await Review.aggregate([
    {
      $match: {
        userId: mongoose.Types.ObjectId(this._id),
        isActive: true,
        deletedAt: null,
      },
    },
...
// Method untuk upgrade ke seller
userSchema.methods.upgradeToSeller = async function (sellerData) {
  if (this.role === "seller") {
  ...
  // Create seller profile menggunakan SellerService
  const SellerService = require("../services/seller/seller.service");
  return await SellerService.createSellerProfile(this._id, sellerData);
};

// Method untuk check apakah user bisa upgrade ke seller
userSchema.methods.canUpgradeToSeller = function () {
  return {
    canUpgrade: this.role === "user" && this.isActive,
    reason:
      this.role === "seller"
        ? "Already a seller"
        : this.role === "admin"
...
userSchema.statics.findWithWallet = function (query = {}) {
  return this.find(query).populate("wallet").populate("profile").populate("sellerProfile");
};

// Method untuk get users dengan wallet statistics
userSchema.statics.getUsersWithWalletStats = async function (options = {}) {
  const { page = 1, limit = 20, role, hasWallet, minBalance, maxBalance, search } = options;
  ...
  const pipeline = [
    ...
    // Add wallet info
    {
      $addFields: {
        ...
        walletBalance: {
          $cond: [{ $gt: [{ $size: "$wallet" }, 0] }, { $arrayElemAt: ["$wallet.balance", 0] }, 0],
        },
      },
    },
